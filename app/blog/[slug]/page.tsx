...
function markdownToHtml(markdown: string): string {
  let html = markdown
    ...
    .replace(/(<li.*<\/li>)\n(<li)/g, '$1$2')
    // Blockquotes
    .replace(/^>\s+(.*$)/gim, '<blockquote class="border-l-4 border-cyan-500 pl-4 my-4 text-slate-300 italic">$1</blockquote>')
    // Code blocks
    .replace(/```([\s\S]*?)```/g, '<pre class="bg-slate-800 rounded-lg p-4 overflow-x-auto my-4"><code class="text-sm text-slate-300">$1</code></pre>')
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <article className="max-w-4xl mx-auto px-4 py-8">
        ...
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1 text-sm bg-slate-800 text-slate-300 px-3 py-1 rounded-full"
              >
        ...
        {/* CTA */}
        <div className="mt-12 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 rounded-xl p-8 border border-slate-800">
          ...
          <Link
            href="/#booking"
            className="inline-block bg-cyan-500 hover:bg-cyan-600 text-white font-semibold px-6 py-3 rounded-lg transition"
          >
